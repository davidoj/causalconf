...
Given $(\prob{P}_\square^{\RV{XY}|\RV{D}[M]},\{\prob{P}_{\tilde{\alpha}}^{\RV{D}}\}_A,f)$ with decisions $\RV{D}_M$ and consequences $\RV{Y}_M$, $\RV{X}_M$, if $\prob{P}_\square^{\RV{Y}_M\RV{X}_M|\RV{D}_M}$ is causally contractible with response conditional $\prob{P}_\square^{\RV{Y}_0\RV{X}_0|\RV{D}_0\RV{H}}$ such that $\RV{Y}_i\CI_{\prob{P}_\square} \RV{D}_i|\RV{HX}_i$ for all $i\in M$, then a causally contractible conditional probability $\prob{P}_\square^{\RV{Y}_M|\RV{X}_M}$ exists. If $\RV{D}_M$ is deterministically controllable, then $\RV{Y}_i\CI_{\prob{P}_\square} \RV{D}_i|\RV{HX}_i$ is also necessary for the existence of $\prob{P}_\square^{\RV{Y}_M|\RV{X}_M}$.
\end{theorem}
...
By assumption, we have the existence of conditionals $\prob{P}_\square^{\RV{Y}_0\RV{X}_0|\RV{D}_0\RV{H}}$ and $\prob{P}_\square^{\RV{X}_0|\RV{H}\RV{D}_0}$. We can conclude that $\prob{P}_\square^{\RV{Y}_0|\RV{X}_0\RV{D}_0\RV{H}}$ also exists, as it is a higher order conditional with respect to $\prob{P}_\square^{\RV{Y}_0\RV{X}_0|\RV{D}_0\RV{H}}$.

Let $\alpha_d\in A$ be such that $\prob{P}_{\alpha_d}^{\RV{D}_0}=\delta_{d}$. By the existence of $\prob{P}_\square^{\RV{Y}_0|\RV{X}_0\RV{H}}$, it follows that
\begin{align}
    \prob{P}_{\alpha_d}^{\RV{Y}_0|\RV{X}_0\RV{H}}(B|x,h) &=  \prob{P}_{\alpha_d}^{\RV{Y}_0|\RV{X}_0\RV{H}}(B|x,d,h)\\
    &\overset{\prob{P}_{\alpha_d}}{\cong} \prob{P}_{\square}^{\RV{Y}_0|\RV{X}_0\RV{H}}(B|x,h)
\end{align}
Because $d$ was arbitrary, this must hold for all $d\in D$.
\end{proof}

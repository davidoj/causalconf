...
For an example, consider some Causal Bayesian Network defining the ``interventional'' probability $\prob{P}(\RV{Y}|do(\RV{X}=x))$ \citep[chap. 1.3]{pearl_causality:_2009}. A Causal Bayesian Network can be a probabilistic model of a decision problem, though it does not have to be. As is typically understood, $\prob{P}(\RV{Y}|do(\RV{X}=x))$ is not known prior to seeing data, and may not even be known after seeing a large amount of data. Finally, $\prob{P}(\RV{Y}|do(\RV{X}=x))$ is often interpreted as ``the probability distribution of $\RV{Y}$, if I were to take some action that sets $\RV{X}$ to be equal to $x$''. Implicit in this interpretation is that the model assigns $\RV{Y}$ the same probability distribution \emph{whatever} action is take that sets $\RV{X}$ equal to $x$. Were this not so, the model would have to specify alternative versions $\prob{P}(\RV{Y}|do_1(\RV{X}=x))$ and $\prob{P}(\RV{Y}|do_2(\RV{X}=x))$ and so forth.

...
We assume that the reason to construct causal models is to help solve decision problems, and that we therefore have a fundamental set of choices $C$ to be compared. For each choice $c\in C$, we assume that some measurement procedure is carried out yielding results in a sample space $(\Omega,\sigalg{F})$, and we represent uncertain knowledge about the result of this procedure with a probability distribution on $(\Omega,\sigalg{F})$. The model postulated is a ``probability function''; a map from $C$ to probability distributions on a sample space $\Omega$. Probability functions are also the kind of model used in many interventional causal models \citet{pearl_causality:_2009,richardson2013single}, as well as the kind of model used in less widespread decision theoretic or Bayesian causal models \citet{dawid_decision-theoretic_2020,lattimore_causal_2019}.

Both decision theoretic causal models and interventional causal models are represented by probability functions, so the diffrence between them is ``only'' is a difference of interpretation. However, the difference in interpretation does lead to differences in the way that we construct models for a given problem.

For example, interventional causal models are typically models of what \citet{dawid_decision-theoretic_2020} calls ``generic variables''. If we have a sequence of independent and identically distributed variables $(\RV{X}_1,\RV{Y}_1),(\RV{X}_2,\RV{Y}_2),...$ that are associated with a concrete measurement procedure, sometimes the individual variables are replaced by a generic ``$\RV{X}$'' and ``$\RV{Y}$''.

Our approach, on the other hand: if the measurement procedure we want to model is associated with the variables $(\RV{X}_1,\RV{Y}_1),(\RV{X}_2,\RV{Y}_2),...$, then we construct a model of those variables. The assumption that the sequence is independent and identically distributed doesn't even make sense when the model is a probability function instead of a single probability distribution.

When we construct a model like this, then it makes sense to ask the question: when \emph{could} we represent the model of the whole sequence with a model of generic variables?

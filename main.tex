...
Thirdly, we regard probabilistic causal models as models of our uncertain knowledge of the outcomes of the different choices facing us. In contrast to approaches to causal graphical models that look for causal relationships ``in nature'' that license certain certain conclusions such as identifiability, we look for features of our state of knowledge that license similar conclusions. For example, a causal model might feature permutable identifiers (Definition \ref{def:ident_permutability}) when, for example, patients are assigned identification numbers we assess that our model should treat these as if they are completely arbitrary beyond their one-to-one connection with a particular patient. ``Arbitrariness'' is a feature of our state of knowledge; someone familiar with individual patients and their identification numbers would not treat these numbers as if they were completely arbitrary.  This approach stands in contrast to interpretations of probabilistic causal models expressing causal relationships that exist ``in nature'' \citet{pearl_does_2018}.
...
\todo[inline]{Roadmap/table of notation}

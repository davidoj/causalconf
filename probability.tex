...
\subsection{Why are variables functions?}

\todo[inline]{I don't think this subsection actually says much about what is coming next}
...
One way to view vague variables is as an existential claim: there is \emph{some} sample space and \emph{some} function that behaves exactly like the thing I have vaguely indicated in the definition. If you and I start measuring people's heights and I tell you ``$\RV{X}_i$ represents the $i$th person's height'', then we can go and measure someone's height and in the end we both somehow decide decide that the appropriate value of $\RV{X}_1$ is, say, $1.78$ metres. Whatever actually happens to get from ``let's measure people's height'' to $\RV{X}_1=1.78$ is extremely hard to explain, but if we can be confident of always agreeing on the end result then the definition appears to have been a success. Furthermore, given that we both come to the same conclusion based on observing the same experiment perhaps this observation process, however it actually works, can be modeled with a some function on some sample space.
...
Suppose more generally that we have a collection of things that can be evaluated by multiple evaluation processes (I can check the value of the things, you can check the value, the department of value checking can check the value), and we're sure that all the evaluation processes will agree on the result in the end. Then we can model this collection of things as a collection of functions, and we can choose a domain that is practical for this modelling task. If all we care about are variables $\RV{X}$ with codomain $X$ and $\RV{Y}$ with codomain $Y$, and they can mutually take any pair of values, then $X\times Y$ might be sufficient. 

We want to say more about our collection of functions, what we want to say might be uncertain and we might use probability to express these uncertain views. However, if we are using probability to model our ``collection of functions'', then our probability model should minimally assign zero probability to collections of outcomes that the function definitions indicate are impossible.
...
We will be following the general approach discussed above, assuming that there is some measurable sample space $\Omega$ and that variables are measurable functions defined on $\Omega$. It is also often standard to assume that we have a \emph{probability space} $(\prob{P},(\Omega,\sigalg{F}))$, where $\prob{P}$ is a $\sigma$-additive measure on $(\Omega,\sigalg{F})$ with $\prob{P}(\Omega)=1$. Given a probability space, the normal way to define the probability distribution of a particular random variable is via the \emph{pushforward measure}. Given $(\prob{P},(\Omega,\sigalg{F}))$ and $f_{\RV{X}}:(\Omega,\sigalg{F})\to (X,\sigalg{X})$, the pushforward measure is $\prob{P}^{\RV{X}} (A) := \prob{P}(\RV{X}^{-1}(A))$ for $A\in \sigalg{X}$.

We will show that a key property of the pushforward measure is that it is the only measure that, in a certain sense, is consistent with both $\prob{P}$ and the function definitions we have used to model our variables. For causal models, we are interested in what \citet{constantinou_extended_2017} refer to as ``non-stochastic variables''. That is, we might be interested in some variable $\RV{D}$ and a probability model that depends on $\RV{D}$ but does not define ``the distribution of $\RV{D}$''. If we do not define the distribution of $\RV{D}$ via the pushforward, we still need probability models that are consistent with the functions we use to model our variables but this is no longer guaranteed by the pushforward rule. We introduce the assumption of \emph{consistency} that reduces to the pushforward rule in the familiar case and also ensures models of nonstochastic variables are consistent.

Consistency has another application. Causal inference also makes use of non-standard operations involving conditional probabilities, and it is useful to have a condition to check when the result of such non-standard operations is a viable model. To illustrate this point, consider ``truncated factorisation'', a linchpin operation in causal graphical models. Suppose we have a causal Bayesian network $(\prob{P}^{\RV{XYZ}},\mathcal{G})$ where $\mathcal{G}$ is a Directed Acyclic Graph that features the edges $\RV{X}\longrightarrowRHD \RV{Y}$ and $\RV{X}\longleftarrowRHD \RV{Z} \longrightarrowRHD \RV{Y}$. Then the result of ``setting $\RV{X}$ to $x$'' is represented by a new probability measure $\prob{P}_x$ that is required to obey the truncated factorisation \citep[page ~24]{pearl_causality:_2009}:
\begin{align}
	\prob{P}^{\RV{XYZ}}_{x}(x',y,z)=\prob{P}^{\RV{Y|XZ}}(y|x,z)\prob{P}^{\RV{Z}}(z)\llbracket x=x'\rrbracket\label{eq:truncated_fac}
\end{align}

There is no special siginificance in standard probability theory to the expression on the right hand side of Equation \ref{eq:truncated_fac}. There are two ways to view this property. First, we can think of starting with knowledge of $\prob{P}_x$ for every $x\in X$ and, if they happen to have this property, then they are a causal Bayesian network relative to $(\prob{P},\mathcal{G})$. On the other hand, we could think of starting with $(\prob{P}^{\RV{XYZ}},\mathcal{G})$ and defining $\prob{P}_x^{\RV{XYZ}}$ to be such that Equation \ref{eq:truncated_fac} holds. Now we have something unusual going on -- we have the irregular expression on the right hand side defining a probability distribution \emph{over the same variables} $\RV{X}$, $\RV{Y}$ and $\RV{Z}$ as the original $\prob{P}^{\RV{XYZ}}$ that we used to construct the expression. However, in this case 


Here we consider in general when it is possible to combine irregular collections of conditional or marginal probabilities (we refer to these as \emph{submodels}) and get a probability model \emph{over the same variables}.

The main challenge with combining submodels is that, as we will show, this operation is ``unsafe'' in the sense that the combination of two well-formed submodels might return a submodel that is not itself well-formed. For a simple example of this, suppose in Equation \ref{eq:truncated_fac} that $\RV{X}$ represents a person's body mass index while $\RV{Z}=(\RV{W},\RV{H})$ represents the pair of their weight and height. The collection $\{\prob{P}^{\RV{XYZ}}_x|x\in X\}$ then models a situation where we change a patient's body mass index while leaving their weight and height unchanged, which is not a particularly useful model, and does not abide by any reasonable definition of ``a variable representing body mass index''.

We discuss some conditions under which the result of composition is well-formed, which includes the case where the variables in question are surjective and \emph{variationally independent}, and if we have a strictly positive model over the same variables we know to be well-formed.
...
Given a \emph{sample space} $\Omega$, a variable $f_\RV{X}$ is a function $\Omega\to A$. We will also refer to the associated Markov kernel $\RV{X}:\Omega\kto A$
...
An indexed Markov kernel $(\kernel{K}',\RV{X},\RV{Y})$ is a \emph{model} if it is \emph{consistent}, which means it assigns probability 0 to contradictions:
\begin{align}
...
Consistency has some interesting implications. For a start, given a probability model of the sample space, there is a unique corresponding probability model of $\RV{X}$ is given by the pushforward of $\RV{X}$ (corollary \ref{corr:pushforward}). Thus it motivates the definition of the pushforward measure as the distribution of $\RV{X}$ induced by the sample space model.

Consistency also implies that for any $\model{K}:\RV{X}\kto\RV{Y}$, if $f_{\RV{Y}}=g\circ f_{\RV{X}}$ then $\model{K}_x^{g(x)}=1$. A particularly simple case of this is a model $\model{L}:\RV{X}\kto\RV{X}$, which must be such that $\model{L}_x^x=1$. \citet{hajek_what_2003} has pointed out that standard definitions of conditional probability allow the conditional probability to be arbitrary on a set of measure zero, even though ``the probability $\RV{X}=x$, given $\RV{X}=x$'' should obviously be 1.

The consistency condition is also needed to ensure sample space models with nontrival domains such as $\model{K}:\RV{X}\kto \text{Id}_\Omega$ conform to expectations imposed by the random variables. We might want to consider such models if we want to include ``nonstochastic variables'' \citep{constantinou_extended_2017}.
...
\begin{lemma}[Pushforward models]\label{lem:pushforward}

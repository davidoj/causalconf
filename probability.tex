...
Probability gap models are probability models that are missing key parts. If we provide a ``missing part'' or ``insert'' of the right type then we get a probability model. We can think of probability as functions that map inserts to probability models, or as higher-order functions that map inserts to lower order probability gap models.

Not every function of this type is a probability gap model; the probability model you get in the end has to break into a ``fixed part'', which stays the same regardless of the insert provided and additionally the insert. For a concrete example: a conditional probability $\prob{P}^{\RV{Y}|\RV{X}}$ is a kind of probability gap model, and when we provide an insert $\prob{Q}_\alpha^{\RV{X}}$ we get a probability model $\prob{P}_\alpha^{\RV{XY}}$ which depends on the $\prob{Q}_\alpha^{\RV{X}}$ chosen. In turn, $\prob{P}_\alpha^{\RV{XY}}$ contains the insert $\prob{Q}_\alpha^{\RV{X}}$ via marginalisation and the fixed $\prob{P}^{\RV{Y}|\RV{X}}$ via disintegration.

Probability gap models must be \emph{valid}. A conditional probability $\prob{P}^{\RV{Y}|\RV{X}}$ is a Markov kernel $X\kto Y$, but not every Markov kernel $X\kto Y$ can be extended to a probability model. Those that can extend to a probability model, given a valid insert, are valid probability gap models.
...
We can reason from graphical properties of model disintegrations to graphical properties of models as Theorem \ref{th:cons_ci}. A general theory akin to d-separation for string diagrams may facilitate a more general theory of appliccability of conditional independence properties.

\begin{theorem}\label{th:cons_ci}
Given a probability 2-comb $\model{P}^{\RV{XV}|\RV{W}\square \RV{Y}|\RV{XV}}$, $\RV{Y}\CI_{\model{P}}^2\RV{V}|\RV{WX}$ if and only if there is a version of $\disint{P}^{\RV{Y}|\RV{XVW}}$ and some $\disint{P}^{\RV{Y}|\RV{WX}}$
\end{theorem}

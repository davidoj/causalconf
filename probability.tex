...
This might seem like an absurd model, but in fact an analogous causal graph appears in \citet{shahar_association_2009} where $\RV{Z}=(\RV{H},\RV{W})$, representing a person's height and weight, and $\RV{X}$ represents their body mass index, which is to say $\RV{X}=\frac{\RV{W}}{\RV{H}^2}$. Furthermore, this paper uses this model to argue that body mass index cannot have a causal effect, which is to say that the causal effect of $\RV{X}$ was a key quantity of interest in this model.
...
Given $\RV{W}:\Omega\to W$, $\RV{X}:\Omega\to X$, $\RV{Y}:\Omega\to Y$, $\RV{Z}:\Omega\to Z$, a probability 2-comb $\prob{P}^{\RV{X}|\RV{W}\square\RV{Z}|\RV{Y}}:W\times Y\kto X\times Z$ is a Markov kernel such that there exist $\prob{P}^{\RV{X}|\RV{W}}:W\kto X$ and $\prob{P}^{\RV{Z}|\RV{WXY}}:W\times X\times Y\kto Z$ such that
\begin{align}
	\prob{P}^{\RV{X}|\RV{W}\square\RV{Z}|\RV{Y}} &= \tikzfig{probability_2_comb}
\end{align}
\end{definition}

The definition of a 2-comb in \citet{jacobs_causal_2019} requires only that $\prob{P}^{\RV{X}|\RV{W}}$ exists, and the existence of $\prob{P}^{\RV{Z}|\RV{WXY}}$ is proven by the existence of kernel disintegrations. We always want to use $\prob{P}^{\RV{Z}|\RV{WXY}}$ so we bake it into the definition.
...
Given $(\Omega,\sigalg{F})$, $\RV{W}:\Omega\to W$, $\RV{X}:\Omega\to X$, $\RV{Y}:\Omega\to Y$ and $\RV{Z}:\Omega\to Z$ a \emph{valid probability 2-comb} $\prob{P}^{\RV{X}|\RV{W}\square\RV{Z}|\RV{Y}}$ is a Markov kernel $Y\kto X\times Z$ such that for all valid $\prob{P}_\alpha^{\RV{Y}|\RV{WX}}$, $\text{insert}(\prob{P}_\alpha^{\RV{Y}|\RV{WX}},\prob{P}^{\RV{X}|\RV{W}\square\RV{Z}|\RV{Y}})$ is valid.
\end{definition}
...
Given $(\Omega,\sigalg{F})$, $\RV{W}:\Omega\to W$, $\RV{X}:\Omega\to X$, $\RV{Y}:\Omega\to Y$ and $\RV{Z}:\Omega\to Z$, a probability 2-comb $\prob{P}^{\RV{X}|\RV{W}\square\RV{Z}|\RV{Y}}$ is valid if $\prob{P}^{\RV{X}|\RV{W}}$ and $\prob{P}^{\RV{Z}|\RV{WXY}}$ are valid.
\end{theorem}

\begin{proof}
Because
\begin{align}
	\text{insert}(\prob{P}_\alpha^{\RV{Y}|\RV{WX}},\prob{P}^{\RV{X}|\RV{W}\square\RV{Z}|\RV{Y}}) = \prob{P}^{\RV{X}|\RV{W}}\odot \prob{P}_\alpha^{\RV{Y}|\RV{WX}} \odot \prob{P}^{\RV{Z}|\RV{WXY}}
\end{align}
The validity of $\prob{P}^{\RV{X}|\RV{W}\square\RV{Z}|\RV{Y}}$ follows from Lemma \ref{lem:valid_extendability}
\end{proof}

The validity of $\prob{P}^{\RV{Z}|\RV{WXY}}$ is not necessary for validity of $\prob{P}^{\RV{X}|\RV{W}\square\RV{Z}|\RV{Y}}$. For example, $\prob{P}^{\RV{Z}|\RV{WXY}}$ may be arbitrary on some set of $\prob{P}^{\RV{X}|\RV{W}}$ measure 0.

We may be able to extend this theory to $n$-combs, where valid $n$-combs are those that can have $n-1$-combs inserted to yield valid $n-1$-combs. We don't develop that theory here, though.

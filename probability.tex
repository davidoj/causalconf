...
Given $(\Omega,\sigalg{F})$, a probability gap model is a pair $(\prob{P}_\square,\{\prob{P}_{\tilde{\alpha}}\}_A)$ where $\prob{P}_\square$ is a probability set and $\{\prob{P}_{\tilde{\alpha}}\}_A$ is a collection of probability sets. A probability gap model induces a map $\prob{P}_\cdot:A\to \mathscr{P}(\Delta(\Omega))$ by
\begin{align}
	\prob{P}_\alpha&:=\prob{P}_\cdot(\alpha)\\
...
\begin{definition}[Conditional probability model]\label{def:cprob_model}

...
\end{align}
for any valid $\RV{X}$ probability distribution $\prob{P}_\alpha^{\RV{X}}$.
\end{definition}
...
An order 1 probability gap model $\prob{P}$ is defined by variables $\RV{W}:\Omega\to W$, $\RV{X}:\Omega\to X$, $\RV{Y}:\Omega\to Y$ and $\RV{X}:\Omega\to Z$ and a valid conditional probability 2-comb $\disint{P}^{\RV{X}|\RV{W}\square\RV{Z}|\RV{Y}}$, and $\prob{P}$ is the map from valid $\RV{Y}|\RV{X}$ conditional probabilities to valid $(\RV{X},\RV{Y},\RV{Z})|\RV{W}$ conditional probabilities given by
\begin{align}
...
We can equivalently define an order 2 probability gap model by the pair $\disint{P}^{\RV{X}|\RV{W}}$ and $\disint{P}^{\RV{Z}|\RV{WXY}}$.

Just as in Section \ref{sec:validity_of_gapprob}, a key concern for probability 2-combs is \emph{valid extendability}. The insert operation yields a valid conditional probability given any valid inputs (Theorem \ref{th:extension_2comb_valid}).
...
A useful property of probability gap models is \emph{recursive disintegration}. If we have a probability gap model $\prob{P}$ and a disintegration $\disint{P}^{\RV{Y}|\RV{X}}$, then defining a new order 1 probability gap model $\prob{Q}$ represented by $\disint{Q}^{\RV{Y}|\RV{X}}=\disint{P}^{\RV{Y}|\RV{X}}$, we can show that disintegrations of $\prob{Q}$ are themselves disintegrations of $\prob{P}$.

\begin{theorem}[Recursive disintegration]\label{th:recursive-disint}
Suppose we have a fundamental probability set $\Omega$, a set of inserts $A$, variables $\RV{W}:\Omega\to W$, $\RV{X}:\Omega\to X$ and $\RV{Y}:\Omega\to Y$, $\RV{Z}:\Omega\to Z$ and a probability gap model $\prob{P}:A\to \Delta(\Omega)$ such that $\disint{P}^{\RV{X}|\RV{Y}}$ is a $\RV{Y}|\RV{X}$ disintegration of $\prob{P}$. Define an order 1 probability gap model $\prob{Q}$ represented by $\disint{Q}^{\RV{Y}|\RV{X}}=\disint{P}^{\RV{Y}|\RV{X}}$. Then if $\disint{Q}^{\RV{Z}|\RV{W}}$ is a $\RV{Z}|\RV{W}$ disintegration of $\prob{Q}$, it is also a $\RV{Z}|\RV{W}$ disintegration of $\prob{P}$.
\end{theorem}
...
\begin{theorem}[Valid disintegration choice]\label{th:valid_disint}
...
Conditional independence has a familiar definition in probability models. For models with probability gaps, we propose the convention that a conditional independence statement for such a model is a claim that conditional independence holds for the whole set of probability models it is associated with. That is, $\RV{B}$ is conditionally independent of $\RV{A}$ given $\RV{C}$ just when that conditional independence holds for all possible extensions to a standard probability model. This convention could also be applied to other kinds of incomplete probability models beyond the hierarchy proposed here.

...
\begin{proof}
\begin{align}
	(\prob{K}\odot \prob{L})\odot \prob{M} &= \tikzfig{odot_\alphassoc_1}\\
											&=  \tikzfig{odot_\alphassoc_2}\\
...
Is some Markov kernel $X\kto Y$, which is equal to $\prob{P}^{\RV{Y}|\RV{X}}$ if $\RV{Y}^X\CI |\RV{X}$. The random variable $\RV{Y}^X$ is often regarded as more than just a curried representation of outcomes, it is taken to express something like ``the outcomes that would have been observed, had $\RV{X}$ turned out differently''. Some authors have argued it is advantageous not to consider such interpretations unless they can be equivalently stated as assumptions about observable phenomena \citet{richardson2013single,dawid_causal_2000}, an approach we will stick to here.
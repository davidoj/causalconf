...
The sample space $(\Omega,\sigalg{F})$ along with the measurement procedure $\proc{S}$ and the associated model variable $\RV{S}$ is a ``model skeleton''. The criterion of \emph{compatibility with observation} establishes a relation between observations and elements of $\sigalg{F}$.
...
\begin{definition}[Uniform higher order conditional distribuition with respect to a probability set]\label{def:ho_cprob_pset}
...
We define \emph{uniform conditional independence} with respect to a probability set as conditional independence for every probability model in the set.

\begin{definition}[Uniform conditional independence]
For a \emph{probability set} $\model{P}_{\{\}}$ and variables $\RV{A},\RV{B},\RV{C}$, we say $\RV{B}$ is uniformly conditionally independent of $\RV{A}$ given $\RV{C}$, written $\RV{B}\CI_{\model{P}_{\{\}}}\RV{A}|\RV{C}$, if for all $\prob{P}_{\alpha}\in \prob{P}_{\{\}}$ $\RV{B}\CI_{\prob{P}_\alpha} \RV{A}|\RV{C}$.
\end{definition}

It is straightforward to show that uniform conditional independence satisfies the semi-graphoid axioms by application of Lemma \ref{lem:distribute_quantifier}
...
\begin{lemma}
Given a standard measurable space $(\Omega,\sigalg{F})$ and $\prob{P}_{\{\}}$ on $\Omega$, uniform conditional independence with respect to $\prob{P}_{\{\}}$ satisfies the semi-graphoid axioms.
\end{lemma}
...
Note that $(\forall \prob{P}_\alpha\in \prob{P}_{\{\}}: f(\prob{P}_{\alpha}))$ is, by definition, a uniform conditional independence statement with respect to $\prob{P}_{\{\}}$.
\end{proof}
